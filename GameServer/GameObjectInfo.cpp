...
GameObjectInfo::GameObjectInfo(GameRoomRef gameRoom, int32 uuid, int32 code): _gameRoomRef(gameRoom), _uuid(uuid), _damage(0), _code(code), _collider(0.f, 0.f)
{
}
...
void GameObjectInfo::SetObjecteState(ObjectStateType state)
{
    _state = state;
}
